...
using System.Text;
using BitVectorWith64Bit = JensUngerer.BitVector.BitVector;
...
namespace JensUngerer.BitVectorSet
{
    public class BitVectorSet<T> : ICollection<T>, ISet<T>
    {
        ...
        private BitVectorWith64Bit BitVector { get; }
        ...
        public BitVectorSet(List<T> elements, Dictionary<T, int> elementsMap, int size = 32)
        {
            ...
            BitVector = new BitVectorWith64Bit(size);
        }
        ...
        private BitVectorWith64Bit Create(IEnumerable<T> other)
        {
            var created = new BitVectorWith64Bit(this.BitVector.Size);
        ...
        private int getIntersectionCardinality(BitVectorWith64Bit otherBitVector)
        {
        ...
        private int getUnionCardinality(BitVectorWith64Bit otherBitVector)
        {
            var ownClonedBitVector = this.BitVector.Clone();
        ...
        public override string ToString()
        {
            ...
            BitVectorWith64Bit.StartString(stringBuilder);
            foreach (int globalIdex in BitVector)
            {
                var element = Elements[globalIdex];
                BitVectorWith64Bit.AppendEntry(stringBuilder, element);
            }
            BitVectorWith64Bit.EndString(stringBuilder);
